...
class UnaryOperatorNode(OperatorNode):
    def __init__(self, oneNode, sym):
        super().__init__(sym)
        self.oneNode = oneNode

    def to_string(self):
        return "(" + self.symbol + " " + self.oneNode.to_string() + ")"
    ...
    def __eq__(self, other):
        if self.__class__ != other.__class__: return False
        if self.symbol != other.symbol: return False
        if self.oneNode != other.oneNode: return False
        return True


# operador ++
class PlusPlusNode(UnaryOperatorNode):
    def __init__(self, oneNode):
        super(PlusPlusNode, self).__init__(oneNode, "++")

    def eval(self):
        return self.oneNode.eval() + 1
...
class MinusMinusNode(UnaryOperatorNode):
    def __init__(self, oneNode):
        super(MinusMinusNode, self).__init__(oneNode, "--")
